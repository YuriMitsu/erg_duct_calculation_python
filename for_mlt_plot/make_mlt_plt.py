...
rmlatmltdata = pyt.get_data('erg_orb_l2_pos_rmlatmlt')
lmdata = pyt.get_data('erg_orb_l2_pos_Lm')
ax1.scatter(rmlatmltdata.y[:,2] * 2*np.pi / 24, lmdata.y[:,1], c='lightgray', s=1, label='2017-04-01')
# ax2.scatter(lmdata.y[:,1], rmlatmltdata.y[:,1], c='gray', s=1)
...
ax1.scatter(deg[mlatmask],l_val[mlatmask],s=1)
...
ax2.scatter(l_val[mlatmask], mlat[mlatmask],s=1)
ax2.set_xlabel('Lm')
ax2.set_ylabel('mlat')
ax2.grid()
...
ax1.legend()
fig.tight_layout()
plt.savefig('/Users/ampuku/Documents/duct/code/python/for_mlt_plot/event_mlt_201704', transparent=False)
